...
fasm68k:
	...
	sleep 1
	git diff --no-index vasm_artifacts/SpeedrunTower.hex fasm68k_artifacts/SpeedrunTower.hex | wc -l > diffing_lines.txt
...
diff:
	git diff --no-index vasm_artifacts/SpeedrunTower.hex fasm68k_artifacts/SpeedrunTower.hex

diff-count:
	git diff --no-index vasm_artifacts/SpeedrunTower.hex fasm68k_artifacts/SpeedrunTower.hex | wc -l
